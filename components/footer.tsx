...
const footerLinks = {
  ...
  formations: [
    ...
    { label: "Packs de Formations", href: "/packs" },
    { label: "Services Freelance", href: "/services" },
  ],
  ressources: [
...
export function Footer() {
  return (
    <footer className="border-t border-[#2D4A6E] bg-[#1A3C6E] text-white">
      <div className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid gap-12 md:grid-cols-2 lg:grid-cols-5">
            ...
            <div className="flex flex-col gap-3">
              ...
              {phoneNumbers.map((phone) => (
                <div key={phone.number} className="flex items-center gap-2 text-sm text-white/60">
                  ...
                  <a href={phone.href} target={phone.isWhatsApp ? "_blank" : undefined} rel={phone.isWhatsApp ? "noopener noreferrer" : undefined} className="hover:text-white transition-colors">
                    {phone.number}
                    {phone.isWhatsApp && <span className="ml-1 text-xs text-[#25D366]">(WhatsApp)</span>}
                  </a>
                </div>
              ))}
            </div>
          ...
          {/* Navigation */}
          <div>
            <h3 className="mb-4 text-sm font-semibold uppercase tracking-wider text-[#F4A620]">
              Navigation
            </h3>
            <ul className="flex flex-col gap-2">
              {footerLinks.navigation.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-sm text-white/60 transition-colors hover:text-[#F4A620]"
                  >
                    {link.label}
                  </Link>
                </li>
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/10 pt-8 md:flex-row">
          ...
          <div className="flex items-center gap-4">
            ...
            <a href="https://github.com" target="_blank" rel="noopener noreferrer" aria-label="GitHub" className="text-white/40 transition-colors hover:text-[#F4A620]">
            ...
            <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-white/40 transition-colors hover:text-[#F4A620]">
